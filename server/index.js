...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/workunit/create') {
    ...
    try {
      ...
      const source = String(body?.source ?? '年度计划').trim() || '年度计划'
      if (!name) {
  ...
  if (url.pathname === '/api/nodes/list') {
    ...
    try {
      ...
      try {
        ...
        console.error('[nodes/list] invalid json', { body: text?.slice(0, 500) })
        sendJson(res, 500, { error: 'Notion 返回解析失败。' })
        return
      }
      const results = Array.isArray(data?.results) ? data.results : []
      ...
      const filtered = results.filter((page) => {
        ...
        const ids = propRelationIds(rel).map((x) => toHyphenId(x).replace(/-/g, ''))
  ...
  if (url.pathname === '/api/ai') {
    ...
    try {
      ...
      if (!apiKey) {
        // No key configured – return mock numbered list
        sendJson(res, 200, {
          result: '1. 强化合规操作培训，提升员工规范意识\n2. 开展专项培训，聚焦业务薄弱环节\n3. 结合案例教学，加深实操理解\n4. 建立培训考核机制，确保学习效果\n5. 定期复盘培训成果，持续改进',
        })
        return
      }
